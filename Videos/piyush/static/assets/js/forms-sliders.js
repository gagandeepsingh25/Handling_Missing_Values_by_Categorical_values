...
(function () {
  const sliderBasic = document.getElementById('slider-basic'),
    ...
    sliderPipsUpdate = document.getElementById('sliderPipsUpdate');
...
if (sliderPipsUpdate) {
  noUiSlider.create(sliderPipsUpdate, {
    start: [0.5],
    behaviour: 'tap-drag',
    step: 0.1,
    tooltips: false,
  ...
  // Manually update the innerText values of the labels
  const labels = sliderPipsUpdate.querySelectorAll('.noUi-value');
  labels.forEach(label => {
    const dataValue = label.getAttribute('data-value');
    label.innerText = dataValue;
  });

  const radiusAmountElement = document.getElementById('radiusAmount');
  const radiusAmountInput = document.getElementById('temperature');

  // Update #radiusAmount dynamically on slider change
  sliderPipsUpdate.noUiSlider.on('update', function (values, handle) {
    radiusAmountElement.innerText = Number(values[handle]).toFixed(1);
    radiusAmountInput.value = Number(values[handle]).toFixed(1);
  });
}
