...
chartData = document.getElementById("chart-data").getAttribute("data-chart_data");
chartData = JSON.parse(chartData.replace(/'/g, '"'));
horizontalBarChartData = document.getElementById("horizontal-bar-chart").getAttribute("data-last_7_days_data");
horizontalBarChartData = JSON.parse(horizontalBarChartData.replace(/'/g, '"'));

function initiateLatestVisitChart() {
    // Bar Chart
// --------------------------------------------------------------------
    ...
    if (barChart) {
        const barChartVar = new Chart(barChart, {
            type: 'bar',
            data: {
                labels: chartData.map((data) => data.chatbot_name),
                datasets: [
                    {
                        data: chartData.map((data) => data.engagements),
                    ...
                    {
                        data: chartData.map((data) => data.visits),
    ...
    if (horizontalBarChart) {
        const horizontalBarChartVar = new Chart(horizontalBarChart, {
            ...
            data: {
                ...
                datasets: [
                    {
                        data: horizontalBarChartData.map((data) => data.engagements),
                        backgroundColor: orangeLightColor,
                        borderColor: 'transparent',
                        maxBarThickness: 15,
                        label: 'Engagements'
                    ...
                    {
                        data: horizontalBarChartData.map((data) => data.leads),
...
function get_data(filter) {
    $.ajax({
        ...
        success: function (data) {
            console.log("response", data.chatbot_data)
            chartData = data.chatbot_data
            document.getElementById('barChart').remove()
            let canvas = document.createElement('canvas');
            canvas.id = 'barChart';
            canvas.className = 'chartjs';
            canvas.setAttribute('data-height', '400');
            canvas.setAttribute('height', '400')
            document.getElementById('barChartWrapper').appendChild(canvas);
