...
(function () {
    ...
    const topVisitorsCount = JSON.parse(data.top_visitors_count.replace(/'/g, "\""));
    const topVisitorsID = JSON.parse(data.top_visitors_id.replace(/'/g, "\""));
    const visitorsByCountry = JSON.parse(data.visitors_by_country.replace(/'/g, "\""));
    const countries = JSON.parse(data.countries.replace(/'/g, "\""));
    const visitorsByDay = JSON.parse(data.visitors_by_day.replace(/'/g, "\""));
    console.log("Data", data)


    // Top Visitors Bar Chart
    // --------------------------------------------------------------------
    const topVisitorsChartEl = document.querySelector('#topVisitorsChart'),
        topVisitorsChartConfig = {
            chart: {
                height: 240,
                type: 'bar',
                parentHeightOffset: 0,
    ...
    if (typeof topVisitorsChartEl !== undefined && topVisitorsChartEl !== null) {
        const topVisitorsChart = new ApexCharts(topVisitorsChartEl, topVisitorsChartConfig);
        topVisitorsChart.render();
    }
    ...
    // const visitors_by_country = document.getElementById("visitors_by_country").value;
    // console.log("visitors_by_country", typeof visitors_by_country)
    // let visitorsByCountry = JSON.parse(JSON.stringify(visitors_by_country));
    // const countries = document.getElementById("countries").value;
    // console.log("countries", typeof countries)
    // const countriesList = JSON.parse(JSON.stringify(countries));
    // console.log("visitorsByCountry", typeof visitorsByCountry)
    // console.log("countriesList", typeof countriesList)
    // get data from django template variable using jinja2
    // visitorsByCountry = '{{ visitors_by_country|safe }}';
